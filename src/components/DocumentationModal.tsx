...
export const DocumentationModal: React.FC<DocumentationModalProps> = ({ isOpen, onClose }) => {
  ...
  const requirements = [
    ...
    { name: 'AI API Integration', status: 'Implemented', detail: 'Google GenAI SDK with GROQ 3.7 Flash model via Express backend' },
    { name: 'Proper Error Handling', status: 'Implemented', detail: 'Structured error messages, loading spinners & graceful fallbacks' },
    { name: 'Secure API Key Handling', status: 'Implemented', detail: 'Server-side process.env.GROQ_API_KEYith zero client token exposure' },
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xs animate-in fade-in duration-150">
      <div className="bg-white rounded-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto shadow-xl border border-slate-200">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
              ...
              <div className="p-3.5 rounded-lg bg-slate-50 border border-slate-200">
                ...
                <p className="text-slate-500 text-[11px] leading-normal">Google GenAI SDK with GROQ 3.7 Flash using deterministic JSON schemas.</p>
              </div>
            </div>
          </div>
       ...
       All GROQ API operations run exclusively on the Express backend via <code className="text-indigo-200 bg-slate-800 px-1 py-0.5 rounded font-mono text-[10px]">process.env.GROQ_API_KEY</code>. The client browser communicates only via secure JSON endpoints (<code className="text-indigo-200 font-mono text-[10px]">/api/GROQ/*</code>) and never receives access to private keys or authorization headers.
            </p>
          </div>
        </div>
