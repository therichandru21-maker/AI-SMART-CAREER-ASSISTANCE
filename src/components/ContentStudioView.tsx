...
export const ContentStudioView: React.FC<ContentStudioViewProps> = ({ onSendToChat }) => {
  const [template, setTemplate] = useState<ContentGenRequest['template']>('email');
  const [topic, setTopic] = useState('');
  const [keyPoints, setKeyPoints] = useState('');
  const [tone, setTone] = useState<ContentGenRequest['tone']>('professional');
  const [length, setLength] = useState<ContentGenRequest['length']>('standard');
  const [audience, setAudience] = useState('Enterprise Executives & Stakeholders');

  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [generatedResult, setGeneratedResult] = useState<ContentGenResponse | null>(null);
  const [copied, setCopied] = useState(false);

  const templates = [
    { id: 'email', label: 'Business Email', icon: Mail, desc: 'Polished client/team correspondence' },
    { id: 'article', label: 'Article / Post', icon: FileText, desc: 'Structured blog or thought piece' },
    { id: 'agenda', label: 'Meeting Agenda', icon: Calendar, desc: 'Timed objectives & preparation' },
    { id: 'bug_report', label: 'Bug Report', icon: Bug, desc: 'Detailed reproduction & severity' },
    { id: 'pitch', label: 'Pitch Deck', icon: Rocket, desc: 'Problem, solution, ROI & CTA' },
    { id: 'social_post', label: 'Social / LinkedIn', icon: Share2, desc: 'Hook, value points & hashtags' },
    { id: 'code', label: 'Code Spec', icon: Code, desc: 'Architecture, clean code & docs' },
    { id: 'freeform', label: 'Custom Freeform', icon: PenTool, desc: 'Flexible creative drafting' },
  ];

  const quickTopicSuggestions: Record<string, { topic: string; keyPoints: string; audience: string }> = {
    email: {
      topic: 'Request budget approval for Q4 Cloud Infrastructure scaling',
      keyPoints: '- 35% user growth requiring 2 extra GPU inference clusters\n- Estimated cost: $12,400/month\n- Projected payback period: 3 months with 99.99% SLA guarantee',
      audience: 'Chief Financial Officer & VP Infrastructure',
    },
    article: {
      topic: 'The Shift to Hybrid AI Workflows in Enterprise Productivity for 2026',
      keyPoints: '- Real-time multi-turn agents vs batch summarizers\n- Security boundaries: Server-side token isolation\n- Measuring tangible productivity ROI in knowledge worker hours',
      audience: 'Product Managers & Engineering Leaders',
    },
    agenda: {
      topic: 'Weekly Product Strategy & Sprint Prioritization Sync',
      keyPoints: '- Review last sprint velocity and QA blockers (15 min)\n- Walkthrough AI Smart Assistant UX overhaul (20 min)\n- Q4 feature roadmap vote & assignments (25 min)',
      audience: 'Core Engineering & Design Team',
    },
    bug_report: {
      topic: 'Memory leak and socket reconnect loop on mobile client disconnect',
      keyPoints: '- Occurs when network transitions from WiFi to LTE\n- CPU spike to 95% on background daemon\n- Steps: launch app, toggle airplane mode 3 times',
      audience: 'Backend & Mobile Infrastructure Engineers',
    },
    pitch: {
      topic: 'AI Smart Assistant: Unified Knowledge Worker Co-Pilot',
      keyPoints: '- Problem: 4.2 hours wasted daily manually parsing memos and writing updates\n- Solution: Instant summarization, deep document inspection, and template studio\n- Business Model: $29/seat/mo B2B SaaS with 82% margin',
      audience: 'Early Stage Venture Capitalists & Angel Investors',
    },
    social_post: {
      topic: 'Announcing our new AI Smart Assistant platform powered by Gemini 3.7 Flash',
      keyPoints: '- Sub-350ms response times for deep document analysis\n- 100% server-side token security\n- Open source template for developer productivity',
      audience: 'Software Engineers, Founders & AI Enthusiasts',
    },
    code: {
      topic: 'TypeScript middleware for caching Gemini API responses with TTL and hash keys',
      keyPoints: '- In-memory LRU cache or Redis support\n- SHA-256 hash generation of prompt & system instruction\n- Cache-Control headers and cache hit telemetry',
      audience: 'Senior Fullstack TypeScript Developers',
    },
    freeform: {
      topic: 'Write a persuasive memo proposing a 4-day work week pilot program',
      keyPoints: '- Productivity data from UK and Japanese trials\n- Clear KPI metrics for client satisfaction and feature delivery\n- 6-month trial with bi-weekly retrospectives',
      audience: 'Executive Committee & HR Leadership',
    },
  };
  ...
  const downloadMarkdown = () => {
    ...
    a.href = url;
    a.download = `${template}-content-${Date.now()}.md`;
    a.click();
  ...
  return (
    <div id="content-studio-view" className="space-y-6">
      ...
      <header className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Content Generation</h2>
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-0.5">
            Generate high-impact business correspondence, articles, meeting agendas, and technical specs.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2">
        {templates.map((t) => {
          ...
          return (
            ...
            >
              <Icon className={`w-4 h-4 ${isSelected ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400 dark:text-slate-500'}`} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 space-y-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 shadow-xs flex flex-col space-y-3 transition-colors">
            ...
            <div>
              <label htmlFor="topic-input" className="block text-[11px] font-medium text-slate-500 dark:text-slate-400 mb-1">
            ...
            <div>
              <label htmlFor="keypoints-input" className="block text-[11px] font-medium text-slate-500 dark:text-slate-400 mb-1">
            ...
            <div className="pt-2 border-t border-slate-100 dark:border-slate-800 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div>
                  <label htmlFor="tone-select" className="block text-[11px] font-medium text-slate-500 dark:text-slate-400 mb-1">
                    Tone & Persona
                  </label>
                  <select
                    id="tone-select"
                  ...
                  >
                    ...
                    <option value="technical">Technical & Precision-Focused</option>
                    <option value="urgent">Urgent & Direct</option>
                  </select>
                </div>

                <div>
                  <label htmlFor="content-length-select" className="block text-[11px] font-medium text-slate-500 dark:text-slate-400 mb-1">
                    Length
                  </label>
                  <select
              ...
              <div>
                <label htmlFor="content-audience-input" className="block text-[11px] font-medium text-slate-500 dark:text-slate-400 mb-1">
                  Target Audience
                </label>
                <input
        ...
        <div className="lg:col-span-7 space-y-4">
          {!generatedResult && !isLoading && (
            <div className="bg-white dark:bg-slate-900 rounded-xl p-10 border border-slate-200 dark:border-slate-800 shadow-xs text-center flex flex-col items-center justify-center min-h-[400px] transition-colors">
              ...
              <h3 className="text-sm font-semibold text-slate-800 dark:text-white">Content Studio Ready</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400 max-w-sm mt-1">
                Select a template above or click "Sample Prompt" to generate emails, agendas, or technical specs.
              </p>
            </div>
          )}
          ...
          {isLoading && (
            <div className="bg-white dark:bg-slate-900 rounded-xl p-8 border border-slate-200 dark:border-slate-800 shadow-xs space-y-4 min-h-[400px] flex flex-col justify-center transition-colors">
              <div className="flex items-center space-x-3 text-indigo-600 dark:text-indigo-400">
                ...
                <span className="text-xs font-semibold tracking-wide uppercase">Composing content with Gemini...</span>
              </div>
            ...
            <div className="space-y-4 animate-in fade-in duration-200">
              ...
              <div className="bg-white dark:bg-slate-900 rounded-xl p-5 border border-slate-200 dark:border-slate-800 shadow-xs space-y-3 transition-colors">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 dark:border-slate-800 pb-3">
                  <div>
                    <h3 className="text-sm font-bold text-slate-900 dark:text-white">{generatedResult.title}</h3>
                    <div className="flex items-center space-x-3 mt-1 text-[11px] text-slate-500 dark:text-slate-400">
                      <span className="flex items-center space-x-1">
                        <Clock className="w-3 h-3 text-slate-400 dark:text-slate-500" />
                        <span>{generatedResult.estimatedReadingTime}</span>
                      </span>
                      <span>•</span>
                      <span className="capitalize">{template.replace('_', ' ')}</span>
                      <span>•</span>
                ...
                <StatsBar text={generatedResult.content} />
              </div>
                ...
                {generatedResult.tips && generatedResult.tips.length > 0 && (
                  <div className="bg-white dark:bg-slate-900 rounded-xl p-4 border border-slate-200 dark:border-slate-800 shadow-xs space-y-2 transition-colors">
                    ...
                    <ul className="space-y-1.5 text-xs text-slate-700 dark:text-slate-300">
                      {generatedResult.tips.map((tip, i) => (
