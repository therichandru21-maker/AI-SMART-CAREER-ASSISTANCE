...
export const QuickTransformView: React.FC = () => {
  ...
  const [selectedAction, setSelectedAction] = useState<QuickTransformRequest['action']>('fix_grammar');
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [resultText, setResultText] = useState<string | null>(null);
  const [copied, setCopied] = useState(false);

  const tools: { id: QuickTransformRequest['action']; label: string; icon: any; desc: string; category: string }[] = [
    { id: 'fix_grammar', label: 'Fix Grammar & Typos', icon: Wrench, desc: 'Correct syntax, punctuation and spelling', category: 'Polish' },
    { id: 'bulletify', label: 'Convert to Bullets', icon: List, desc: 'Transform messy paragraphs into structured lists', category: 'Structure' },
    { id: 'make_formal', label: 'Make Formal & Executive', icon: Briefcase, desc: 'Elevate tone for corporate leadership', category: 'Tone' },
    { id: 'simplify_eli5', label: 'Explain Like I\'m 5', icon: Smile, desc: 'Simplify complex jargon into plain English', category: 'Clarity' },
    { id: 'to_table', label: 'Convert to Markdown Table', icon: Table, desc: 'Structure comparisons or raw data into a table', category: 'Structure' },
    { id: 'extract_checklist', label: 'Extract Action Checklist', icon: CheckSquare, desc: 'Isolate actionable todos with checkboxes', category: 'Action' },
    { id: 'translate_es', label: 'Translate: Spanish', icon: Languages, desc: 'Accurate and natural Spanish translation', category: 'Translate' },
    { id: 'translate_fr', label: 'Translate: French', icon: Languages, desc: 'Fluent French translation', category: 'Translate' },
    { id: 'translate_de', label: 'Translate: German', icon: Languages, desc: 'Professional German translation', category: 'Translate' },
    { id: 'translate_ja', label: 'Translate: Japanese', icon: Languages, desc: 'Polite Japanese translation', category: 'Translate' },
  ];

  const handleTransform = async (actionToRun?: QuickTransformRequest['action']) => {
    ...
    try {
      ...
      if (!res.ok) {
        const errData = await res.json().catch(() => ({ error: 'Transformation failed' }));
  ...
  return (
    <div id="quick-transform-view" className="space-y-6">
      {/* Top Header & Actions */}
      <header className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Text Transformation</h2>
        ...
        {tools.map((tool) => {
          ...
          return (
            <button
              ...
              className={`p-3 rounded-lg border text-left flex flex-col justify-between space-y-2 transition cursor-pointer ${
                isSelected
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span className={`text-[10px] uppercase font-bold tracking-wider px-1.5 py-0.2 rounded ${
                  isSelected ? 'bg-indigo-100 dark:bg-indigo-900/60 text-indigo-700 dark:text-indigo-300' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400'
                }`}>
                  {tool.category}
                </span>
              </div>
              <div>
                ...
                <div className={`text-[10px] line-clamp-1 mt-0.5 ${isSelected ? 'text-indigo-600/80 dark:text-indigo-300/80' : 'text-slate-400 dark:text-slate-500'}`}>
                  {tool.desc}
                </div>
              </div>
            </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 shadow-xs flex flex-col space-y-3 transition-colors">
            <div className="flex items-center justify-between pb-2 border-b border-slate-100 dark:border-slate-800">
              ...
              <span className="text-[10px] text-slate-400 dark:text-slate-500 font-medium">
                Action: {tools.find(t => t.id === selectedAction)?.label}
              </span>
            </div>
            ...
            <textarea
              ...
              rows={10}
              className="w-full text-xs text-slate-800 dark:text-slate-100 bg-slate-50/50 dark:bg-slate-800/60 p-3 rounded-lg border border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 font-mono leading-relaxed resize-y"
            />
        ...
        {/* Output Pane */}
        <div className="lg:col-span-6 space-y-3">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 shadow-xs space-y-3 min-h-[380px] flex flex-col transition-colors">
            <div className="flex items-center justify-between pb-2 border-b border-slate-100 dark:border-slate-800">
              <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">
                Transformed Output
              </span>
            ...
            <div className="flex-1 flex flex-col justify-center">
              {!resultText && !isLoading && (
                <div className="text-center py-12 text-slate-400 dark:text-slate-500 space-y-2">
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400">Select any transformation tool above to execute.</p>
                </div>
              )}
              ...
              {isLoading && (
                <div className="py-12 text-center text-slate-500 dark:text-slate-400 space-y-3">
                  ...
                  <p className="text-xs font-medium">Transforming syntax with Gemini...</p>
                </div>
              )}

              {resultText && !isLoading && (
                <div className="space-y-3 animate-in fade-in duration-150">
                  <div className="p-4 rounded-lg bg-slate-50/50 dark:bg-slate-800/40 text-xs text-slate-800 dark:text-slate-100 leading-relaxed font-sans prose-sm dark:prose-invert max-w-none border border-slate-100 dark:border-slate-800">
